...
# Racing : solid kerbs, no collision clouds
...
func add_clouds(x, z, parent):
	...
	if roll < proba_cloud:
		AaPrism.random_free(Vector3(3,3,3), Vector3(x, heigth*cell_size+0.2, z),
				 Vector3(2.23*cell_size, 2.23*cell_size, 2.23*cell_size), parent, m_cloud, false)
...
func add_pivot(x, z, parent):
	...
	if roll < proba_pivot:
		...
		if coin_toss():
			...
			AaPrism.build_above( Vector3(x, 0, z),
					 Vector3(3*cell_size, 0.01*cell_size, 3*cell_size), parent, m_grass)
